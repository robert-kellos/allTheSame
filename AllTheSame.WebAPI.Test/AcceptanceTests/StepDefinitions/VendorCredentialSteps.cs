...
{
    [Binding]
    public class VendorCredentialSteps : BaseServiceTest
        //AuthenticatedTest //- Allows automatic fetching of token for each get call
    {
        ...
        private readonly DateTime _confirmedOn = DateTime.UtcNow;
        ...
        [When(
            @"I call the add VendorCredential Post api endpoint to add a VendorCredential it checks if exists pulls item edits it and deletes it"
            )]
        public void
        ...
        [Then(
            @"the add result should be a VendorCredential Id check exists get by id edit and delete with http response returns"
            )]
