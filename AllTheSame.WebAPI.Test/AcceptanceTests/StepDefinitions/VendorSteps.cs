...
{
    [Binding]
    public class VendorSteps : BaseServiceTest
        //AuthenticatedTest //- Allows automatic fetching of token for each get call
    {
        public override string Uri => "/api/Vendor";
        ...
        [When(
            @"I call the add Vendor Post api endpoint to add a Vendor it checks if exists pulls item edits it and deletes it"
            )]
        public void WhenICallTheAddVendorPostApiEndpointToAddAVendorItChecksIfExistsPullsItemEditsItAndDeletesIt()
        {
        ...
        public void ThenTheAddResultShouldBeAVendorIdCheckExistsGetByIdEditAndDeleteWithHttpResponseReturns()
        {
            ...
            //pull the item by Id
            var resultGet = GetById<Vendor>(_getIdValue);
        ...
        public void GivenTheFollowingVendorAddInput(Table table)
        {
            ...
            {
                _name = row["Name"];
                _orgId = Convert.ToInt32(row["OrgId"]);
