...
{
    [Binding]
    public class ResidentSteps : BaseServiceTest
        //AuthenticatedTest //- Allows automatic fetching of token for each get call
    {
        ...
        [When(
            @"I call the add Resident Post api endpoint to add a Resident it checks if exists pulls item edits it and deletes it"
            )]
        ...
        [Then(
            @"the add result should be a Resident Id check exists get by id edit and delete with http response returns")
        ]
